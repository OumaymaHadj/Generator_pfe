...
export default async function createCrud(
  ...
  try {
    ...
    console.log(greenText, `\n\tCRUD for table ${selectedTable.name} created successfully \u2714.\n`);
  } catch (error) {
...
exports.getAll${capitalizeFirstLetter(name)}s = async function (req, res) {
    try {
        ...
        // Vérifier si des ${name}s existent
        if (!${name}s || ${name}s.length === 0) {
            return res.status(404).json({
                message: 'No ${name}s found',
            });
        }

        // Construire la réponse
        const response = {
            data: ${name}s, // Liste des ${name}s
            primaryKey: "_id", // Assurez-vous que _id est une clé dans vos documents
            isAutoIncrement: false, // MongoDB utilise ObjectId, pas d'auto-incrément
        };

        // Retourner la réponse en JSON
        return res.status(200).json(response);
    } catch (err) {
        console.error('Error getting ${name}s:', err); // Afficher l'erreur dans la console pour le développeur
        return res.status(500).json({
            message: 'An error has occurred',
            error: err.message, // Facultatif : inclure le message d'erreur
        });
    }
};
...
exports.delete${capitalizeFirstLetter(name)} = async function (req, res) {
    try {
        const deleted${capitalizeFirstLetter(name)} = await ${capitalizeFirstLetter(name)}.findOneAndDelete(req.params.id);
